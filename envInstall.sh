...
# Function - install a package if not already installed
install_package() {
  PACKAGE=$1
...
configure_ssh() {
  USERNAME=$(logname)
  ssh_key_path="$USERNAME/.ssh/syncora_id_rsa"
  ...
  while true; do
    read -sp "Enter passphrase for SSH key (required): " passphrase
    echo
  ...
  ssh-keygen -t rsa -b 4096 -f "$ssh_key_path" -N "$passphrase" -C "syncora@$(hostname)" -q
...
echo " ========================================";
echo "                                         ";
echo "Welcome to the environment setup for local development on Syncora"
